...
// رنگ‌های آماده حرفه‌ای + گزینه‌ی «سفارشی» که از accentColor خوانده می‌شود
export type ColorTheme = 'blue' | 'violet' | 'emerald' | 'amber' | 'rose' | 'teal' | 'crimson' | 'custom';
export type TextColor = 'auto' | `#${string}`;
export type Language = 'fa'; // در آینده: | 'en'
export type TradingTimeMode = 'device' | 'broker';
// شعاع گوشه‌ها: از تیز (کاملاً گوشه‌دار) تا خیلی گرد
export type RadiusScale = 'none' | 'sm' | 'md' | 'lg' | 'xl';
// تراکم چیدمان: فاصله‌گذاری و ابعاد قابل‌کلیک در کل رابط کاربری
export type LayoutDensity = 'compact' | 'comfortable' | 'spacious';
// ظاهر پس‌زمینه‌ی سایدبار
export type SidebarStyle = 'solid' | 'glass' | 'gradient';
...
export interface AppSettings {
  ...
  customAccentColor: string; // فقط وقتی colorTheme === 'custom' استفاده می‌شود
  textColor: TextColor;
  radius: RadiusScale;
    ...
    }),
    {
      ...
      merge: (persisted: any, current) => ({
        ...
        fontSize: ['xs', 'sm', 'md', 'lg', 'xl'].includes(persisted?.fontSize)
          ? persisted.fontSize
          : defaults.fontSize,
        colorTheme: ['blue', 'violet', 'emerald', 'amber', 'rose', 'teal', 'crimson', 'custom'].includes(persisted?.colorTheme)
          ? persisted.colorTheme
          : defaults.colorTheme,
        customAccentColor: typeof persisted?.customAccentColor === 'string' && /^#[0-9a-f]{6}$/i.test(persisted.customAccentColor)
          ? persisted.customAccentColor
        ...
        textColor: persisted?.textColor === 'auto'
          ...
          ? persisted.textColor
          : defaults.textColor,
        radius: ['none', 'sm', 'md', 'lg', 'xl'].includes(persisted?.radius)
          ? persisted.radius
          : defaults.radius,
        density: ['compact', 'comfortable', 'spacious'].includes(persisted?.density)
          ? persisted.density
          : defaults.density,
        sidebarStyle: ['solid', 'glass', 'gradient'].includes(persisted?.sidebarStyle)
          ? persisted.sidebarStyle
          : defaults.sidebarStyle,
