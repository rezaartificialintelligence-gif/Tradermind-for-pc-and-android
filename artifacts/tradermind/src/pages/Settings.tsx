...
import {
  ...
  Square, RectangleHorizontal, Rows3, PanelsTopLeft, Sparkles, Droplet
} from "lucide-react";
...
export default function Settings() {
  ...
  const store = useAppStore(
    useShallow(s => ({
       theme: s.theme, fontSize: s.fontSize, colorTheme: s.colorTheme, customAccentColor: s.customAccentColor,
      textColor: s.textColor, radius: s.radius, density: s.density, sidebarStyle: s.sidebarStyle,
      ...
      // setters
       ...
       setColorTheme: s.setColorTheme, setCustomAccentColor: s.setCustomAccentColor, setTextColor: s.setTextColor,
      setRadius: s.setRadius, setDensity: s.setDensity, setSidebarStyle: s.setSidebarStyle,
    ...
    <div className="space-y-5 animate-in fade-in duration-500 max-w-2xl mx-auto" dir="rtl">
      ...
      <Section icon={Sun} title={t.settings.appearance} description={t.settings.appearanceDesc}>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2 sm:grid-cols-7">
            {[
              ...
              { value: 'crimson', label: 'زرشکی', color: '#dc2626' },
          ...
          {/* رنگ‌بندی سفارشی — هر رنگ دلخواه برای هویت بصری برند شما */}
          <div className={cn(
            'mt-2 flex items-center justify-between gap-3 rounded-lg border-2 p-2.5 transition-all',
            ...
            >
              <Droplet className={cn('w-4 h-4 shrink-0', store.colorTheme === 'custom' ? 'text-primary' : 'text-muted-foreground')} />
              <span className={store.colorTheme === 'custom' ? 'text-foreground' : 'text-muted-foreground'}>رنگ سفارشی</span>
            </button>
            <div className="flex items-center gap-2 shrink-0">
              <input
                type="color"
                value={/^#[0-9a-f]{6}$/i.test(store.customAccentColor) ? store.customAccentColor : '#3b82f6'}
                onChange={e => { store.setCustomAccentColor(e.target.value); store.setColorTheme('custom'); }}
                className="h-8 w-10 cursor-pointer rounded-md border border-border bg-transparent p-1"
                aria-label="انتخاب رنگ اصلی سفارشی"
              />
              <span className="text-[11px] font-mono text-muted-foreground uppercase" dir="ltr">
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-2">
            <div>
              <p className="text-sm font-medium">شعاع گوشه‌ها</p>
              <p className="text-xs text-muted-foreground mt-0.5">حالت گوشه دکمه‌ها، کارت‌ها و کادرهای ورودی</p>
            </div>
            <Square className="w-4 h-4 text-primary shrink-0" />
          </div>
          <div className="grid grid-cols-5 gap-2">
            {[
              { value: 'none', label: 'تیز', r: '0px' },
              { value: 'sm', label: 'کم', r: '4px' },
              { value: 'md', label: 'متوسط', r: '8px' },
              { value: 'lg', label: 'زیاد', r: '12px' },
              { value: 'xl', label: 'خیلی گرد', r: '18px' },
            ].map(({ value, label, r }) => (
              <button
                key={value}
                type="button"
                onClick={() => store.setRadius(value as any)}
                className={cn(
                  'flex flex-col items-center gap-1.5 border-2 p-2 text-[11px] font-medium transition-all rounded-lg',
                  store.radius === value
                    ? 'border-primary bg-primary/10 text-primary'
                    : 'border-border hover:border-primary/40 text-muted-foreground'
                )}
                aria-label={`شعاع گوشه: ${label}`}
              >
                <span className="h-6 w-6 border-2 border-current" style={{ borderRadius: r }} />
                {label}
              </button>
            ))}
          </div>
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-2">
            <div>
              <p className="text-sm font-medium">تراکم چیدمان</p>
              <p className="text-xs text-muted-foreground mt-0.5">فاصله‌گذاری داخلی کارت‌ها و ردیف‌های تنظیمات</p>
            </div>
            <Rows3 className="w-4 h-4 text-primary shrink-0" />
          </div>
          <div className="grid grid-cols-3 gap-2">
            {[
              { value: 'compact', label: 'فشرده', desc: 'اطلاعات بیشتر در یک نگاه', icon: Rows3 },
              { value: 'comfortable', label: 'معمولی', desc: 'تعادل پیش‌فرض', icon: PanelsTopLeft },
              { value: 'spacious', label: 'راحت', desc: 'خوانایی بیشتر', icon: RectangleHorizontal },
            ].map(({ value, label, desc, icon: Icon }) => (
              <button
                key={value}
                type="button"
                onClick={() => store.setDensity(value as any)}
                className={cn(
                  'flex flex-col items-center gap-1 rounded-lg border-2 p-2.5 text-xs font-medium transition-all text-center',
                  store.density === value
                    ? 'border-primary bg-primary/10 text-primary'
                    : 'border-border hover:border-primary/40 text-muted-foreground'
                )}
        ...
        <div>
          <div className="flex items-center justify-between gap-3 mb-2">
            <div>
              <p className="text-sm font-medium">پس‌زمینه سایدبار</p>
              <p className="text-xs text-muted-foreground mt-0.5">جلوه‌ی ظاهری منوی کناری برنامه</p>
            </div>
            <Sparkles className="w-4 h-4 text-primary shrink-0" />
          </div>
          <div className="grid grid-cols-3 gap-2">
            {[
              { value: 'solid', label: 'ساده', desc: 'یکدست و واضح' },
              { value: 'glass', label: 'شیشه‌ای', desc: 'محو و شفاف' },
              { value: 'gradient', label: 'گرادیانت', desc: 'رنگی و مدرن' },
            ].map(({ value, label, desc }) => (
              <button
                key={value}
                type="button"
                onClick={() => store.setSidebarStyle(value as any)}
                className={cn(
                  'flex flex-col items-center gap-1 rounded-lg border-2 p-2.5 text-xs font-medium transition-all text-center',
                  store.sidebarStyle === value
                    ? 'border-primary bg-primary/10 text-primary'
                    : 'border-border hover:border-primary/40 text-muted-foreground'
                )}
              >
                <span
                  className="h-6 w-10 rounded-md border border-border/60"
                  style={
                    value === 'glass'
                      ? { background: 'linear-gradient(135deg, hsl(var(--sidebar)/0.4), hsl(var(--primary)/0.25))', backdropFilter: 'blur(2px)' }
                      : value === 'gradient'
                      ? { background: 'linear-gradient(180deg, hsl(var(--sidebar)), hsl(var(--primary)/0.35))' }
